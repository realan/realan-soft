...
const FormTemplate = (props) => {
    ...
    const handleOK = () => {
        ...
        if (!!props.initialState.id) { // if "id" defined then update mutation, else add
    ...
    const fieldsList = props.fields.filter(item => item.show !== false)
        .map((item, index) => {
            let fieldItem;
            switch (item.type) {
                case 'text':
                    ...
                    fieldItem = (
                        ...
                        <FormControl fullWidth >
                            <DatePicker
                                ...
                                locale={ru}
                                value={formState[item.accessor] || ""}
                                onChange={ date => {setFormState({ ...formState, [item.accessor]: date }) }}
                ...
                case 'select':
                    ...
                    const selectorItems = selectorList.map((li) => {
                        let menuItemText = showInItem.reduce((text, it) => text + (li[it] !== null ? li[it] : "") + " ", "");
                        return (
                            < MenuItem
                                classes={{
