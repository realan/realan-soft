...
export default function AddOrderDrawer() {
    ...
    const handleChange = (event) => {
        ...
        formState.forEach(x => {
            if (x.type === event.target.name) { x.val = event.target.value }
        })
    };
        ...
        <div className={classes.list} role="dialog" >
            <List>
                <form className={classes.root} noValidate autoComplete="off">
                    ...
                    <Button variant="contained" onClick={toggleDrawer(anchor, false)} >Отмена</Button>
                    <Button variant="contained" color="primary" type="submit" onClick={handleSubmit}>Добавить</Button>
                </form>
            </List>
        </div>
    );
    ...
    if (error) { console.log('error addOrder ', error) }

    return (
        <div>
            <React.Fragment key={anchor}>
                <Button variant="contained" color="primary" onClick={toggleDrawer(anchor, true)}>Новый заказ</Button>
                <Drawer
                    anchor={anchor}
